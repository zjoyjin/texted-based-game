...
class World:
    ...
    Instance Attributes:
        ...
        - locations: a list representation of this items
        - npcs: a list of npc
        - locations: a list representation of this world's location
        ...
        for line in location_data:  # in numerical order (location 0 = locations[0])
            if 'LOCATION' in line:
                ...
                if (current[0] != 40 and current[0] != 41 and current[0] != 37):
                    locs[current[0]] = Location(current[0], current[1], self.get_coords_from_num(current[0]),
                                                current[2], current[3])
                else:
                    locs[current[0]] = Shop(current[0], current[1], self.get_coords_from_num(current[0]), current[2],
                                            current[3])
                current = []
            else:
    ...
    def load_items(self, item_data: TextIO):
        ...
        for line in item_data:
            current = line.split(',')
            itms.append(Item(current[5].strip(), int(current[2]), int(current[3]), int(current[0]), int(current[1]),
                             current[4]))
        return itms

    def load_shops(self):
        for item in self.items:
            if (item.x, item.y) == self.get_coords_from_num(41) or (item.x, item.y) == self.get_coords_from_num(40) or (
                    item.x, item.y) == self.get_coords_from_num(37):
    ...
    def load_npcs(self):
        return [RichLady("Rich Lady", 10, 8, 9), MiserableStudent("Miserable Student", 0, 5, 7),
                CryingGirl("Crying Girl", 0, 7, 4)]
